...
def update_reading(reading_id):
	...
	if 'time' in request.json and type(request.json['time']) is not unicode:
		abort(400)
	reading[0]['value1'] = request.json.get('value1', reading[0]['value1'])
	reading[0]['value2'] = request.json.get('value2', reading[0]['value2'])
	reading[0]['date'] = request.json.get('date', reading[0]['date'])
	reading[0]['time'] = request.json.get('time', reading[0]['time'])
	writecsv()
	return jsonify({'reading': reading[0]})
